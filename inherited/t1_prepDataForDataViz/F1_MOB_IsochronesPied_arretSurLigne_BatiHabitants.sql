-- Input : 
-- nom du schema                                                    - "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"
-- la couche de locaux la plus acutalisée ::  						- fftp_2020_pb0010_local
-- la couche d'isochrones piétons 5, 10 , 15 autour des TAD :: 		- isochrones_5_10_15_pieton_arretsSurLigne
...
drop table if exists "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".pop_cap_2018;
create table "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".pop_cap_2018 as (
with t1 as(
		
			select tt.id_geom insee, tt.geom, pop.total_pop_18::int  
			from public."territory_table_CAP20" tt
			left join "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"."INSEE_population_tot_2018_geo_2021" pop
...
drop table if exists "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"."chiffres_cles_intermediaire";
create table "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"."chiffres_cles_intermediaire" as (
with ff as(
...
select  row_number() OVER () AS id, tt.id_geom , iso."time"::varchar, st_union(st_intersection(tt.geom, st_buffer(iso.geom,50))) geom 
	from "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"."isochrones_5_10_15_pieton_arretsSurLigne" iso
		left join public."territory_table_CAP20" tt
...
rank2 as (
	...
	select final.insee , nb_log::int , poptot::int  , pos , CAST("time"as bigint) from final
		left join "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".pop_cap_2018 pop
...
drop table if exists "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".chiffres_cles ; 
create table "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".chiffres_cles as (
with ratio as (
	with out as(
		select insee,  sum(nb_log)  nb_log from "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".chiffres_cles_intermediaire 
...
from "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".chiffres_cles_intermediaire cc 
	left join ratio
...
DROP TABLE IF EXISTS "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".dataviz_logements ;
CREATE TABLE "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".dataviz_logements AS (
...
--//--//--//--//
DROP TABLE IF EXISTS "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".dataviz_isochrones ;
CREATE TABLE "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants".dataviz_isochrones AS (
select   iso.id::int , st_buffer(iso.geom,50) ,  "time"
	from "F1_MOB_IsochronesPied_arretSurLigne_BatiHabitants"."isochrones_5_10_15_pieton_arretsSurLigne" iso
		,public."territory_table_CAP20" tt
		--on 
	where tt.rank = '3' AND st_intersects(tt.geom, iso.geom)
	ORDER BY "time"
)
