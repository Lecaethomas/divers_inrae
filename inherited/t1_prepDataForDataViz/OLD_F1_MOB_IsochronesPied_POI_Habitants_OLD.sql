...
-- la couche d'isochrones piétons 5, 10 , 15 autour des POI :: 		- isochrones_5_10_15_pieton_arrets_TAD
...
drop table if exists "F1_MOB_IsochronesPied_POI_Habitants".pop_cap_2018;
create table "F1_MOB_IsochronesPied_POI_Habitants".pop_cap_2018 as (
with t1 as(
		
			select tt.id_geom insee, tt.geom, pop.total_pop_18::int  
			from public."territory_table_CAP20" tt
			left join "F1_MOB_IsochronesPied_POI_Habitants"."INSEE_population_tot_2018_geo_2021" pop
...
drop table if exists "F1_MOB_IsochronesPied_POI_Habitants"."chiffres_cles_intermediaire";
create table "F1_MOB_IsochronesPied_POI_Habitants"."chiffres_cles_intermediaire" as (
with ff as(
...
select  row_number() OVER () AS id, tt.id_geom , iso."time"::varchar, st_union(st_intersection(tt.geom, st_buffer(iso.geom,50))) geom 
	from "F1_MOB_IsochronesPied_POI_Habitants"."isochrones_5_10_15_pieton_arrets_TAD" iso
		left join public."territory_table_CAP20" tt
...
rank2 as (
	...
	select final.insee , nb_log::int , poptot::int  , pos , CAST("time"as bigint) from final
		left join "F1_MOB_IsochronesPied_POI_Habitants".pop_cap_2018 pop
...
drop table if exists "F1_MOB_IsochronesPied_POI_Habitants".chiffres_cles ; 
create table "F1_MOB_IsochronesPied_POI_Habitants".chiffres_cles as (
with ratio as (
	with out as(
		select insee,  sum(nb_log)  nb_log from "F1_MOB_IsochronesPied_POI_Habitants".chiffres_cles_intermediaire 
...
select cc.insee as id_geom, ratio.tot_log as "Count_ENTITIES", poptot ,  pos "Location" , time , 
	ratio.tot_log::numeric / poptot::numeric as "ratioLocPop",  
	ratio.tot_log::numeric / poptot::numeric * nb_log as "EquivPop"
from "F1_MOB_IsochronesPied_POI_Habitants".chiffres_cles_intermediaire cc 
	left join ratio
